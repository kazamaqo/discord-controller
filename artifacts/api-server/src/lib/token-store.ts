...
import { installAccountAutomationListener, installPrimaryCommandListener } from "./command-listener";
...
function encryptionKey(): Buffer {
  ...
  if (!secret) {
    throw new Error("TOKEN_ENCRYPTION_KEY, DASHBOARD_PASSWORD, or SESSION_SECRET is required for token encryption");
  }
...
async function loadAccountTokens(): Promise<Map<AccountId, string>> {
  ...
  for (const row of result.rows) {
    if ((ACCOUNT_IDS as readonly string[]).includes(row.account_id)) {
      tokens.set(row.account_id as AccountId, decryptToken(row));
    }
  }
  return tokens;
}
